...
export default function Doctrina() {
  ...
  return (
    <section className="px-[8%] lg:px-[12%] pt-16 pb-20 service relative bg-[#f6f6f6] overflow-hidden">
      {/* --- ENCABEZADO --- */}
      <header className="flex flex-col lg:flex-row gap-10 mb-16 relative z-10">
        <div className="w-full lg:w-1/3 title pt-2">
      ...
      <div className="flex flex-col border-t border-gray-300 relative z-10">
        {doctrinaData.map((item: DoctrinaItem) => {
          ...
          return (
            <article key={item.id} className={`border-b border-gray-300 transition-all duration-300 ${isOpen ? "bg-white" : ""}`}>
              <button
                onClick={() => toggleAccordion(item.id)}
                className="w-full py-10 px-4 flex justify-between items-center text-left"
              >
                <div className="flex items-center gap-6">
                  <span className={`text-4xl CalSans transition-colors ${isOpen ? "text-[#00338d]" : "text-gray-300"}`}>
                    {item.id.padStart(2, '0')}
                  </span>
                  <h3 className={`text-2xl md:text-3xl CalSans transition-colors ${isOpen ? "text-[#00338d]" : "text-gray-800"}`}>
                    {item.title}
                  </h3>
                </div>
                <div className={`text-3xl transition-transform duration-300 ${isOpen ? "rotate-45 text-[#00338d]" : "text-gray-400"}`}>
                  <i className="ri-add-line"></i>
                </div>
              </button>

              <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isOpen ? "max-h-[2000px] pb-16 opacity-100" : "max-h-0 opacity-0"}`}>
                <div className="px-4 lg:pl-36 lg:pr-[15%]">
                  <div 
      ...
      <footer className="mt-12 pt-8 border-t border-gray-200 relative z-10">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 text-gray-400 GolosText text-sm italic">
          <div className="flex items-center gap-2">
            ...
            <p>
              Contenido basado en los <span className="font-bold text-gray-500">Artículos de Fe</span> de la Iglesia Pentecostal Unida de Colombia.
            </p>
          </div>
          <a 
            ...
            className="text-[#00338d] font-bold not-italic hover:underline flex items-center gap-1 transition-all"
          >
            Ver fuente original
