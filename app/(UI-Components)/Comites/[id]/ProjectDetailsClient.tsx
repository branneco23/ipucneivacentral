...
export default function ProjectDetailsClient({ id }: ProjectDetailsProps) {
  const [data, setData] = useState<ComitéData | null>(null);
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center text-slate-500 font-bold">Cargando información desde Firebase...</div>;
  }

  if (!data) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-slate-600 p-6 text-center">
        <h2 className="text-2xl font-bold mb-2">Comité no configurado</h2>
        <p className="text-sm text-slate-400">Aún no se han guardado datos para este comité ({id}) desde el panel de administración en Firebase.</p>
      </div>
    );
  }

  let rawYouTubeIds: string[] = [];
  if (Array.isArray(data.youtubeIds)) {
    rawYouTubeIds = data.youtubeIds;
  } else if (typeof data.youtubeIds === 'string') {
    rawYouTubeIds = (data.youtubeIds as string).split(',').map(s => s.trim());
  }

  const idPrincipalIndex = rawYouTubeIds.findIndex(idStr => idStr && idStr !== "xyz" && idStr !== "");
  const videoPrincipal = idPrincipalIndex !== -1 ? rawYouTubeIds[idPrincipalIndex].split('&')[0] : null;
  const videosSecundarios = rawYouTubeIds
    .filter((idStr, idx) => idStr && idStr !== "xyz" && idx !== idPrincipalIndex)
    .map(idStr => idStr.split('&')[0].trim());
  ...
  return (
    <main className="bg-white min-h-screen selection:bg-blue-100 pt-[90px] lg:pt-[120px]">
      ...
      {/* MODAL GALERÍA */}
      {selectedImageIndex !== null && data.eventos && data.eventos[selectedImageIndex] && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10 text-white">
          ...
          <button onClick={() => setSelectedImageIndex(null)} className="absolute top-6 right-6 z-[110] hover:text-red-500 transition-colors text-3xl font-bold">✕</button>
          <button onClick={(e) => { e.stopPropagation(); prevImage(); }} className="absolute left-4 md:left-10 z-[110] text-white/50 hover:text-white text-5xl">‹</button>
          ...
          <button onClick={(e) => { e.stopPropagation(); nextImage(); }} className="absolute right-4 md:right-10 z-[110] text-white/50 hover:text-white text-5xl">›</button>
        </div>
      )}
      ...
      <section className="relative h-[45vh] min-h-[400px] bg-blue-950 flex items-center justify-center overflow-hidden">
        {data.fotoGrupal && (
          <Image src={data.fotoGrupal} alt={data.nombre || "Comité"} fill priority className="absolute inset-0 object-cover blur-md opacity-25 scale-105" />
        )}
      ...
      <div className="w-full py-20">
        <div className="max-w-[1400px] mx-auto px-6 grid md:grid-cols-2 gap-8 mb-32">
          {data.mision && (
            <div className="bg-slate-50 p-12 rounded-[3rem] border-b-[12px] border-blue-600 shadow-sm">
          ...
          {data.vision && (
            <div className="bg-slate-50 p-12 rounded-[3rem] border-b-[12px] border-orange-500 shadow-sm">
        ...
        {data.integrantes && data.integrantes.length > 0 && (
          <section className="max-w-[1600px] mx-auto px-6 mb-40">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {data.integrantes.map((persona, index) => (
        ...
        {data.historietas && data.historietas.length > 0 && (
          <section className="mb-40 w-full overflow-x-hidden">
            ...
            <div className="space-y-12 w-full">
              {data.historietas.map((hist, idx) => (
                <RevistaLibro key={idx} historia={hist} />
              ))}
            </div>
          </section>
        )}
        ...
        {id === "escuela-dominical" && (
          <section className="max-w-[1400px] mx-auto px-6 mb-32">
            <SopaLetras
              ...
              descripcion="Busca los conceptos que aprendimos en este capítulo."
  ...
  return (
    <div onClick={onClick} className={`relative ${isMain ? 'h-[500px]' : 'aspect-video'} w-full rounded-[3rem] overflow-hidden cursor-pointer group shadow-2xl border-4 border-white bg-slate-900`}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <div className={`${isMain ? 'w-24 h-24' : 'w-16 h-16'} bg-red-600 rounded-full flex items-center justify-center shadow-lg group-hover:scale-110 transition-all duration-500`}>
          <span className="text-white text-3xl font-black">▶</span>
        </div>
